...
public class CenfomonFuego extends Cenfomon {
    private List<IObservador> observers = new ArrayList<IObservador>();
    private static Scanner scan = new Scanner(System.in);
    private boolean flag1 = false;
    private boolean flag2 =false;
    public CenfomonFuego(String nombre, String tipo, int nivel, int HP, int ataque, int defensa, String naturaleza, String mov1, String mov2, String mov3, boolean piedraeterna, boolean metamorfosis2) {
        setNombre(nombre);
        setTipo(tipo);
        setNivel(nivel);
        setHP(HP);
    ...
    public void Evolocionar() throws InterruptedException {
        if(this.getNivel() >12 && this.isPiedraeterna() == false && flag1 == false){
            SecuenciaEvo();
            System.out.println(getNombre()+" ha metamorfoseado a Salamluna");
            this.setNombre("Salamluna");
            this.setMetamorfosis2(true);
            flag1 = true;
        }
        else if(this.getNivel() > 32 && this.isMetamorfosis2() == true && this.isPiedraeterna()==false && flag2 == false){
            SecuenciaEvo();
            System.out.println(getNombre()+" ha metamorfoseado a Dragoluna");
            this.setNombre("Dragoluna");
            flag2 =true;
        }
    }
